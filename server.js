...
if (!fs.existsSync(DATA_FILE)) {
  ...
  fs.writeFileSync(DATA_FILE, JSON.stringify({ 
    appointments: [],
    users: [],  // Новое: пользователи
    support_tickets: []  // Новое: обращения в поддержку
  }, null, 2));
}
...
// Получить записи по userId (для авторизованных пользователей)
...
// ============================================
// НОВЫЕ API: РЕГИСТРАЦИЯ И АВТОРИЗАЦИЯ
// ============================================
...
app.post('/api/register', (req, res) => {
  ...
  if (writeAppointments(data)) {
    console.log('✅ Зарегистрирован пользователь:', name, 'ИНН:', inn);
    res.json({ 
      success: true, 
...
app.post('/api/login', (req, res) => {
  ...
  console.log('🔑 Вход пользователя:', user.name);
  res.json({ 
    success: true, 
...
app.patch('/api/user/:inn', (req, res) => {
  ...
  if (writeAppointments(data)) {
    console.log('✏️ Обновлены данные пользователя:', data.users[userIndex].name);
    res.json({ 
      success: true, 
      user: { 
...
app.post('/api/support', (req, res) => {
  ...
  if (writeAppointments(data)) {
    console.log('📩 Новое обращение от пользователя:', userId);
...
app.patch('/api/support/:id/reply', (req, res) => {
  ...
  data.support_tickets[ticketIndex].adminReply = adminReply;
  data.support_tickets[ticketIndex].status = 'answered';
  data.support_tickets[ticketIndex].answeredAt = new Date().toISOString();
